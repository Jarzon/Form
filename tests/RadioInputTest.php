...
class RadioInputTest extends TestCase
{
    ...
    public function testGetFormsRadio()
    {
        ...
        $this->assertEquals('<input name="test" type="radio" value="test">', $content['test']->html[0]['html']);

        $forms->selected('test');

        $content = $forms->getForms();

        $this->assertEquals('<input name="test" type="radio" value="test" checked>', $content['test']->html[0]['html']);
    }
}